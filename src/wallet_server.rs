...
use crate::chain_server::block_chain::transaction::Transaction;
pub async fn divide_request(req: Request<Body>) -> Result<Response<Body>, Error> {
    match (req.method(), req.uri().path()) {
        (&Method::GET, "/walletServer/testAlive") => { test_alive_handler().await }
        (&Method::GET, "/walletServer/createWallet") => { create_wallet().await }
...
pub async fn create_wallet() -> Result<Response<Body>, Error> {
